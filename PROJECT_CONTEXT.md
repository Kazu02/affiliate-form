...
| 種別 | 説明 |
|------|------|
| アフィリエイト管理SS | メインSS（**正データ**）。`1JOMT_Uuoq3H6O9lZcAKuSpzC2ehUJ35k53mVvnZRwY0`。設定シート（`設定_*`）に設定とフォーム回答を両方格納。「管理」シートに全フォームの`回答数`と`最終回答日時`があり、**代理店コード付きも含む全フォームの受信状況を1枚で確認できる** |
...
- チャットからの実行は一時デプロイの保守ルート:
  `?adv_admin=<キー>&action=preview|rebuild|backup|ensuremonths&months=202607,202608[&backup=1]`
  （`preview` は書き込まずに件数と現状を返す。本番デプロイにはこのルートは載せていない）
- **上書きを伴う再生成の前は必ず `&backup=1`** を付ける（SSまるごとDriveへ複製する）。
- **広告主シートは放置すると必ずメインSSとずれる。** 承認は成果発生より後に付くのに、リアルタイム書き込みは承認=false固定だから。広告主へ提出する前に `action=preview` で差分を見て、ずれていれば対象月を再生成する。
- **件数を数えるときに gviz を使わない。** `gviz/tq?sheet=<名前>` は名前を解決できないと**黙って先頭シートの内容を返す**ため、別の月を読んでいることに気付けない（2026-07-30 に 202604 を誤読）。`action=preview` はA列の値で数えるので正確。
